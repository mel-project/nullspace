...
use crate::screens::settings::form;
...
pub(super) fn render(ui: &mut Ui, app: &mut NullspaceApp) {
    ...
    form::show(ui, "profile_editor", |form| {
        form.row("Display name", |ui| {
            ui.add(
                TextEdit::singleline(&mut *display_name_input).desired_width(ui.available_width()),
            );
        });
    ...
    let can_save = (display_changed || avatar_changed) && !upload_busy && !save_busy.get();
