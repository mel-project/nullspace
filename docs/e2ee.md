...
Before we discuss the specific protocols, it's useful to outline two primitives. 
...
An header-encrypted message is BCS-encoded with the following fields:
...
- `headers`, a list of headers, each of which contains:
    ...
    - `headers[i].receiver_key`, the 32-byte, per-message symmetric key `K` encrypted with `DH(sender_esk, receiver_mpk)` using XChaCha20 with a zero nonce.
...
- `signature`, an ed25519 signature over `body`
...
- Alice header-encrypts the device-signed plaintext to all of Bob's device